...
ifeq ($(GOHOSTOS), windows)
	...
	API_PROTO_GEN_FILES=$(shell bash -c "find api -name *.pb.go")
else
...
api:
	protoc --proto_path=./api \
	       ...
	       $(API_PROTO_FILES)
	cp openapi.yaml /d/openapi-doc/openapi-followup.yaml

# 清理根据proto文件生成的代码
.PHONY:clean
clean:
	rm $(API_PROTO_GEN_FILES)


.PHONY: wire
# wire
wire:
	cd cmd/followup && wire
...
# build
build-linux:
	mkdir -p bin/ && GOOS=linux go build -ldflags "-s -w -X main.Version=$(VERSION)" -o ./bin/ ./...
